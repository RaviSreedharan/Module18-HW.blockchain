...
#   Create a new data class named `Record`. This class will serve as the blueprint 
#   for the financial transaction records that the blocks of the ledger will store.
#
# Step 2: Modify the Existing Block Data Class to Store Record Data
#   Change the existing `Block` data class by replacing the generic `data`
#   attribute with a `record` attribute that is of type `Record`.
#
...
################################################################################
# Streamlit Code - Adds the cache decorator for Streamlit.
...
#   5. As part of the Add Block button functionality, update `new_block` so that `Block` consists of an attribute
#      named `record`, which is set equal to a `Record` that contains the `sender`, `receiver` & `amount` values.
#      The updated `Block`should also include the attributes for `creator_id` and `prev_hash`.

# @TODO:
# Delete the `input_data` variable from the Streamlit interface.
# xxx input_data = st.text_input("Block Data")
